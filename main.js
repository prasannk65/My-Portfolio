...
class Effect {
  ...
  threeInit() {
    ...
    this.controls.autoRotateSpeed = 2; // Slowed down slightly for less nausea behind text
    this.controls.maxDistance = 350;
    this.controls.minDistance = 150;
    this.controls.enablePan = false;
    this.controls.enableZoom = false; // Disable zoom to prevent interfering with scroll? No, Zoom is fine if targeting canvas, but overlay catches events. 
    // Usually, if overlay is on top, orbit controls won't work unless we pass events through or the overlay has pointer-events: none.
    // However, the user might want interaction.
    // I put z-index: 10 on #app. So OrbitControls won't receive events from the center.
    // I will enable pointer-events on the canvas but #app covers it.
    // To allow rotating the background, users have to click on empty spaces or we disable pointer events on non-interactive parts of #app?
    // Let's stick to standard scrolling for now. If they want to rotate, they can grab the edges if visible, but full overlay usually blocks it. 
    // I'll leave it as is.
  }
        ...
        return new Promise((resolve, reject) => {
          textureLoader.load(
            ...
            (error) => {
              ...
              resolve(true); // Resolve anyway to not break Promise.all completely if one fails
            }
          );
        });
      })
  ...
  updateSingleMovingStar(i, idx, easing) {
    // Check if velocitiesStar is initialized
    if (!this.velocitiesStar || !this.positionsStar.array) return;

    // Safety check for indices
    if (idx + 2 >= this.positionsStar.array.length) return;
  ...
  loop() {
    ...
    if (this.scene) {
      // Combined mouse and scroll interaction
      this.scene.rotation.y += 0.05 * (targetX - this.scene.rotation.y);
      this.scene.rotation.x += 0.05 * (targetY + (scrollY * 0.0005) - this.scene.rotation.x);
    }
    ...
    // Check if stars are visible/initialized before updating
